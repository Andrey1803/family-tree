...
class FamilyTreeModel:
    ...
    def load_from_file(self, filename=None):
        if filename is None:
            filename = self.data_file
        print(f"[DEBUG] load_from_file: filename={filename}")
        print(f"[DEBUG] load_from_file: exists={os.path.exists(filename)}")
        ...
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                data = json.load(f)
            print(f"[DEBUG] load_from_file: marriages_in_file={data.get('marriages', [])[:3]}")
            self.persons = {}
            ...
            print(f"[DEBUG] load_from_file: marriages_raw count={len(marriages_raw)}")
            print(f"[DEBUG] load_from_file: marriages_raw sample={marriages_raw[:3]}")
            self.marriages = {}
            ...
            print(f"[DEBUG] load_from_file: загружено браков={len(self.marriages)}")
