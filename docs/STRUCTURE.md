...
## Результат сборки .exe (scripts\build_exe.bat)

| Режим | Выход |
|-------|-------|
| onefile | `dist/Семейное_древо.exe` — один файл, всё внутри |

Папки `dist/Семейное_древо/` (от старого onedir) больше не создаётся. Данные (users.json, family_tree_*.json) создаются рядом с exe при первом запуске.
