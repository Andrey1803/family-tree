...
class FamilyTreeApp:
    def __init__(self, root, data_file=None, username=None):
        ...
        self.file_menu.add_command(label="Резервные копии", command=self.open_backup_dialog)
        self.file_menu.add_separator()
        ...
        # === МЕНЮ ДОБАВЛЕНИЯ РОДСТВЕННИКА (С ЦВЕТОВЫМ ОФОРМЛЕНИЕМ) ===
        ...
        # --- РОДИТЕЛИ ---
        self.add_relative_menu.add_command(
            label="Отец",
            font=("Segoe UI", 9),
            foreground=MALE_FG,
        ...
        self.add_relative_menu.add_command(
            label="Мать",
            font=("Segoe UI", 9),
            foreground=FEMALE_FG,
            activeforeground="#ffffff",
            activebackground="#c41e3a",
        ...
        self.add_relative_menu.add_command(
            ...
            activeforeground="#ffffff",
            activebackground="#1e40af",
            command=lambda: self.add_spouse_dialog(self.last_selected_person_id, "Мужской")
        )
        self.add_relative_menu.add_command(
            label="Супруга",
            font=("Segoe UI", 9),
            ...
            activeforeground="#ffffff",
            activebackground="#c41e3a",
            command=lambda: self.add_spouse_dialog(self.last_selected_person_id, "Женский")
        )

        self.context_menu.add_cascade(label="➕ Добавить родственника", menu=self.add_relative_menu)
        # --- /КОНТЕКСТНОЕ МЕНЮ ---
        # --- СВЯЗИ КЛАВИШ ---
        ...
        self.root.bind("<Control-s>", lambda e: self.save_file())  # Сохранение
        # === КОНЕЦ ИСПРАВЛЕНИЯ ===
        # --- /СВЯЗИ КЛАВИШ ---
        # --- ЗАГРУЗКА ---
        ...
        if BACKUP_AVAILABLE:
            self.backup_manager, self.auto_save_manager = init_backup_system(self.model, self.root)
            backup_info = self.backup_manager.get_backup_info()
            print(f"Бэкапы: {backup_info['count']} шт., автосохранение каждые {backup_info['interval_minutes']} мин.")
        else:
        ...
        if person:
            ...
            if not has_father:
                self.add_relative_menu.add_command(
                    ...
                    activeforeground="#ffffff",
                    activebackground="#1e40af",
                    command=lambda p=self.last_selected_person_id: self.add_parent_dialog(p, "Мужской")
                )
            if not has_mother:
                self.add_relative_menu.add_command(
                    label="Мать",
                    ...
                    activeforeground="#ffffff",
                    activebackground="#c41e3a",
                    command=lambda p=self.last_selected_person_id: self.add_parent_dialog(p, "Женский")
                )
            self.add_relative_menu.add_separator()
            ...
            if person.gender == "Мужской":
                self.add_relative_menu.add_command(
                    ...
                    activeforeground="#ffffff",
                    activebackground="#1e40af",
                    command=lambda p=self.last_selected_person_id: self.add_child_dialog(p, "Мужской")
                )
                self.add_relative_menu.add_command(
                    label="Дочь",
                    font=("Segoe UI", 9),
                    ...
                    activeforeground="#ffffff",
                    activebackground="#c41e3a",
                    command=lambda p=self.last_selected_person_id: self.add_child_dialog(p, "Женский")
                )
            else:
                self.add_relative_menu.add_command(
                    label="Сын",
            ...
            self.add_relative_menu.add_command(
                ...
                activeforeground="#ffffff",
                activebackground="#1e40af",
                command=lambda p=self.last_selected_person_id: self.add_sibling_dialog(p, "Мужской")
            )
            self.add_relative_menu.add_command(
                label="Сестра",
                font=("Segoe UI", 9),
                ...
                activeforeground="#ffffff",
                activebackground="#c41e3a",
                command=lambda p=self.last_selected_person_id: self.add_sibling_dialog(p, "Женский")
            )
            self.add_relative_menu.add_separator()
            ...
            if person.gender == "Мужской":
                self.add_relative_menu.add_command(
                    ...
                    activeforeground="#ffffff",
                    activebackground="#c41e3a",
                    command=lambda p=self.last_selected_person_id: self.add_spouse_dialog(p)
                )
            else:
                self.add_relative_menu.add_command(
                    label="Супруг",
    ...
    def on_exit(self):
        """Обработка закрытия приложения с синхронизацией."""
        ...
        if self.model and self.model.modified:
            result = messagebox.askyesnocancel(
                "Сохранить изменения?",
                "Данные были изменены. Сохранить и синхронизировать с сервером перед выходом?"
            )
            ...
            # Данных нет или нет изменений, но всё равно синхронизируемся при закрытии
            if SYNC_AVAILABLE and self.username:
                sync_result = messagebox.askyesno(
                    "Синхронизация",
                    "Синхронизировать данные с сервером перед выходом?"
                )
                if sync_result:
    ...
    def edit_person(self):
        ...
        # --- Вкладка 4: Фотоальбом (доп. фото) — показываем превью фотографи�� ---
    ...
    def new_file(self):
        if messagebox.askyesno("Подтверждени��",
    ...
    def save_file(self):
        """Сохраняет данные локально и синхронизирует с сервером."""
        # Создаём резервную копию перед сохранением
        if self.backup_manager and self.model.data_file and os.path.exists(self.model.data_file):
            try:
                self.backup_manager.create_backup(self.model.data_file, backup_name_prefix="backup")
            except Exception as e:
        ...
        if success:
            ...
            if SYNC_AVAILABLE and self.username:
                self._sync_with_server()
            try:
                self.statusbar.config(text="Данные сохранены" + (" • Синхронизировано" if SYNC_AVAILABLE else ""))
            except AttributeError:
                pass  # statusbar может отсутствовать
        else:
            try:
                self.statusbar.config(text="Ошибка при сохранении")
            except AttributeError:
                pass
        return success
    ...
    def _sync_with_server(self):
        ...
        if not SYNC_AVAILABLE or not self.username:
            return False
        
        try:
            if sync_to_server(self.model.data_file, self.username):
                self.statusbar.config(text="✓ Синхронизировано с сервером")
                return True
            else:
                self.statusbar.config(text="⚠ Синхронизация не удалась")
                return False
        except Exception as e:
            print(f"Sync error: {e}")
            self.statusbar.config(text="⚠ Ошибка синхронизации")
            return False
    ...
    def open_backup_dialog(self):
        ...
        # Заголовок
        tk.Label(dialog, text="🗄️ Управление резервными копиями", 
                font=("Segoe UI", 14, "bold")).pack(pady=10)
        
        # Информация о бэкапах
        backup_info = self.backup_manager.get_backup_info()
        info_text = (
            f"Всего бэкапов: {backup_info['count']} из {backup_info['max_allowed']}\n"
            f"Последний бэкап: {backup_info['latest'] or 'нет'}\n"
            f"Автосохранение: каждые {backup_info['interval_minutes']} мин."
        )
        tk.Label(dialog, text=info_text, font=("Segoe UI", 10), 
                justify="left").pack(pady=10, padx=20, anchor="w")
        
        # Список бэкапов
        tk.Label(dialog, text="Доступные резервные копии:", 
                font=("Segoe UI", 10, "bold")).pack(pady=(10, 5), padx=20, anchor="w")
        ...
        backup_listbox = tk.Listbox(listbox_frame, height=10, font=("Segoe UI", 9),
        ...
        for backup_path, backup_time in backups:
            display_name = f"{backup_time.strftime('%d.%m.%Y %H:%M')} - {backup_path.name}"
            backup_listbox.insert(tk.END, display_name)
        
        if not backups:
            backup_listbox.insert(tk.END, "Нет доступных резервных копий")
            backup_listbox.itemconfig(0, {'fg': 'gray'})
        ...
        def restore_backup():
            ...
            if not selection:
                messagebox.showwarning("Предупреждение", "Выберите резервную копию для восстановления.")
                return
            
            if backup_listbox.size() == 1 and "нет доступных" in backup_listbox.get(0).lower():
                messagebox.showwarning("Предупреждение", "Нет доступных резервных копий.")
                return
            
            idx = selection[0]
            if idx < len(backups):
                backup_path, _ = backups[idx]
                
                if messagebox.askyesno("Подтверждение", 
                                      f"Восстановить из резервной копии?\n{backup_path.name}\n\n"
                                      "Все текущие изменения будут сохранены в новую резервную копию."):
                    # Создаём бэкап текущего состояния перед восстановлением
                    if self.model.data_file and self.model.modified:
                        self.backup_manager.create_backup(self.model.data_file, "pre_restore")
                    
                    # Восстанавливаем
                    if self.backup_manager.restore_from_backup(self.model.data_file, backup_path):
        ...
        def delete_backup():
            selection = backup_listbox.curselection()
            if not selection:
                messagebox.showwarning("Предупреждение", "Выберите резервную копию для удаления.")
                return
            ...
            if idx < len(backups):
                ...
                if messagebox.askyesno("Подтверждение", 
                    ...
                    try:
                        backup_path.unlink()
                        backup_listbox.delete(idx)
                        messagebox.showinfo("Успех", "Резервная копия удалена.")
                    except Exception as e:
        ...
        def open_backup_folder():
            ...
            import subprocess
            try:
                subprocess.run(['explorer', str(self.backup_manager.backup_dir)], check=True)
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось открыть папку: {e}")
        
        tk.Button(btn_frame, text="Восстановить", command=restore_backup, 
                 bg="#4CAF50", fg="white", width=15).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame, text="Удалить", command=delete_backup, width=15).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame, text="Открыть папку", command=open_backup_folder, width=15).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame, text="Закрыть", command=dialog.destroy, width=15).pack(side=tk.LEFT, padx=5)
    ...
    def _compare_versions(self, v1, v2):
        """Сравнивает версии. Возвращает: -1 (v1 < v2), 0 (равны), 1 (v1 > v2)."""
        def normalize(v):
    ...
    def _show_update_dialog(self, parent_dialog, new_version, release_notes):
        ...
        msg = (
            ...
            "⚠️ Перед обновлением будет создана резервная копия всех данных."
        )
        ...
        if messagebox.askyesno("Доступно обновление", msg, parent=self.root):
            ...
            if backup_path:
                confirm = messagebox.askyesno(
                    ...
                    f"Продолжить установку версии {new_version}?",
                    parent=self.root
                )
                if confirm:
                    self._download_and_install_update(new_version, parent_dialog)
            else:
    ...
    def _create_backup(self):
        ...
        try:
            backup_dir = os.path.join(os.path.dirname(self.model.data_file), "backups")
            os.makedirs(backup_dir, exist_ok=True)
            
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            backup_name = f"family_tree_backup_{timestamp}.json"
            backup_path = os.path.join(backup_dir, backup_name)
            
            # Копируем основной файл
            if os.path.exists(self.model.data_file):
                shutil.copy2(self.model.data_file, backup_path)
                
                # Копируем файлы пользователей
                user_settings = f"user_settings_{self.username}.json" if self.username else "user_settings.json"
                if os.path.exists(user_settings):
                    shutil.copy2(user_settings, os.path.join(backup_dir, f"user_settings_backup_{timestamp}.json"))
    ...
    def _download_and_install_update(self, new_version, parent_dialog=None):
        ...
        try:
            ...
            msg = (
                f"📥 Скачивание версии {new_version}...\n\n"
                "Открываю страницу релиза в браузере.\n"
                "Скачайте установочный файл и запустите его.\n\n"
                "✅ Ваши данные сохранены в резервной копии."
            )
            messagebox.showinfo("Загрузка обновления", msg, parent=self.root)
            
            # Открываем страницу релиза в браузере
            release_url = f"https://github.com/Andrey1803/family-tree/releases/tag/v{new_version}"
            webbrowser.open(release_url)
            
            if parent_dialog:
                self.update_status_label.config(text=f"📥 Версия {new_version} доступна для загрузки", fg="#2196F3")
    ...
    def _show_developer_dialog(self):
        ...
        # Заголовок
        tk.Label(dialog, text="👨‍💻 Разработчик", 
                font=("Segoe UI", 16, "bold")).pack(pady=15)
        
        # Информация
        info_frame = tk.Frame(dialog)
        info_frame.pack(fill="both", expand=True, padx=40)
